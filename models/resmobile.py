...
class BasicBlock(nn.Module):
    # class constructor
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1, expansion: int = 1, downsample: nn.Module = None, mnet_conv: bool = False):
        ...
        self.conv1 = make_convolution(in_channels, out_channels, kernel_size=3, 
                                      stride=stride, padding=1, bias=False, mnet_conv=mnet_conv)
        ...
        self.conv2 = make_convolution(out_channels, out_channels*self.expansion, kernel_size=3, 
                                      stride=1, padding=1, bias=False, mnet_conv=mnet_conv)
...
class ResNet(nn.Module):
    def __init__(self, img_channels: int, num_layers: int, block: Type[BasicBlock], num_classes: int  = 1000, mnet_conv: bool = False):
        ...
        self.conv1 = make_convolution(img_channels, self.in_channels, 
                                      kernel_size=7, stride=2, padding=3, bias=False, mnet_conv=mnet_conv)
    ...
    def _make_layer(self, block: Type[BasicBlock], out_channels: int, blocks: int, stride: int = 1) -> nn.Sequential:
        ...
        """ eg: Resnet18 to understand logic of basic blocks, downsampling, shortcut connections (see figure 3 of ResNet paper).
          ...
          Fourth layer: ("conv4_x")
            first block: output_layer3 => conv(3x3, 256, /2) => conv(3x3, 256) => x' + downsample(output_layer3) + ...        Below adds a convolutional (downsample) layer to the end of a BasicBlock
        ...
        if stride != 1 or self.in_channels != out_channels*self.expansion: 
            downsample = nn.Sequential(
                make_convolution(self.in_channels, out_channels*self.expansion, 
                                 kernel_size=1, stride=stride, padding=0, bias=False, mnet_conv=self.mnet_conv),
                nn.BatchNorm2d(out_channels * self.expansion),
            )

        # Add the first basic block. If `downsample` is not None, it will be added to the start of the block.
        layers = []
        layers.append(block(self.in_channels, out_channels, stride, self.expansion, downsample, mnet_conv=self.mnet_conv))
        
        # Stack the remaining BasicBlocks
        self.in_channels = out_channels * self.expansion
        for _ in range(1, blocks):
            layers.append(block(self.in_channels, out_channels, expansion=self.expansion, mnet_conv=self.mnet_conv))
